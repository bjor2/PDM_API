...
namespace PDM_API.Controllers
{
    ...
    public class WellBoreInjDayController : ControllerBase
    {
        private PDMContext _db;
        private IUserProvider _userProvider;
        readonly ILogger _logger;

        public WellBoreInjDayController(PDMContext context, IUserProvider userProvider, ILoggerFactory loggerFactory)
        {
            _db = context;
            _userProvider = userProvider;
            _logger = loggerFactory.CreateLogger("WellBoreInjDayController");
        }
        ...
        [HttpGet]
        [ProducesResponseType(typeof(WellBoreInjDay), 200)]
        public async Task<IActionResult> GetWellBoreInjDay(int? skip, int? top, DateTime? START_PROD_DAY, DateTime? END_PROD_DAY, string PERF_CODE, DateTime? WB_V_START_DATE, string DBSOURCE_ID, string WELL_BORE_CODE, string GOV_FCTY_NAME)
        {
            ...
            var paramSource = new SqlParameter("@sourceView", "[PDM].[WB_INJ_DAY]");
            // t = source table/view, w = WELL_MASTER
            var paramJoinColumn = new SqlParameter("@joinColumn", "W.FCTY_CODE = t.FCTY_CODE");

            try
            {
                var response = _db.WellBoreInjDay.FromSql("EXEC [PDM].[P_GetDataWithRowLevelSecurity] @userName, @top, @skip, @filter, @sourceView, @joinColumn", paramUserName, paramTop, paramSkip, paramFilter, paramSource, paramJoinColumn);

                return Ok(await response.ToArrayAsync());
            }
            catch (Exception e)
            {
                _logger.LogError("Something went wrong in WellBoreInjDayController: " + e.Message);
                throw;
            }
        }
    }
