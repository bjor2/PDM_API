...
namespace PDM_API.Controllers
{
    ...
    public class WellInjDayCompactController : ControllerBase
    {
        readonly ILogger _logger;
        private PDMContext _db;

        public WellInjDayCompactController(ILogger<WellInjDayCompactController> logger, PDMContext context)
        {
            _logger = logger;
            _db = context;
        }
        ...
        [HttpGet]
        [ProducesResponseType(typeof(WellInjDayCompact), 200)]
        public async Task<IActionResult> GetWellInjDayCompact(int? skip, int? top, string columns, DateTime? START_PROD_DAY, DateTime? END_PROD_DAY, string INJ_TYPE, string DBSOURCE_ID, string WELL_UWI, string GOV_FCTY_NAME)
        {
            try
            {
                 ...
                 */
                var result = await _db.WellInjDay
                    .Where(e =>
                        ...
                        && (WELL_UWI == null || WELL_UWI == "" || WELL_UWI == e.WELL_UWI)
                        && (GOV_FCTY_NAME == null || GOV_FCTY_NAME == "" || GOV_FCTY_NAME == e.GOV_FCTY_NAME)                    
                        )
                    .Skip(s)
                    .Take(t)
            ...
            {
                _logger.LogError($"Something went wrong in WellInjDayCompactController: {e.Message}. Stacktrace: {e.StackTrace}");
