...
namespace PDM_API.Controllers.Stream
{
    ...
    public class WellActivityController : ControllerBase
    {
        private PDMContext _db;
        private IUserProvider _userProvider;
        readonly ILogger _logger;

        public WellActivityController(PDMContext context, IUserProvider userProvider, ILoggerFactory loggerFactory)
        {
            _db = context;
            _userProvider = userProvider;
            _logger = loggerFactory.CreateLogger("WellActivityController");
        }
        ...
        /// <returns></returns>
        [HttpGet]
        [ProducesResponseType(typeof(WellActivity), 200)]
        public async Task<IActionResult> GetWellActivity(int? skip, int? top)
        {
            ...
            var paramSource = new SqlParameter("@sourceView", "[PDM].[WELL_ACTIVITY]");
            ...
            {
                _logger.LogError("Something went wrong in GetWellActivity: " + e.Message);
                throw;
            }
        }
    }
