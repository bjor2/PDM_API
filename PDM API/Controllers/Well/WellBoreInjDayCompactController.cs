...
namespace PDM_API.Controllers
{
    ...
    public class WellBoreInjDayCompactController : ControllerBase
    {
        readonly ILogger _logger;
        private PDMContext _db;

        public WellBoreInjDayCompactController(ILogger<WellBoreInjDayCompactController> logger, PDMContext context)
        {
            _logger = logger;
            _db = context;
        }
        ...
        [HttpGet]
        [ProducesResponseType(typeof(WellBoreInjDayCompact), 200)]
        public async Task<IActionResult> GetWellBoreInjDayCompact(int? skip, int? top, string columns, DateTime? START_PROD_DAY, DateTime? END_PROD_DAY, string PERF_CODE, DateTime? WB_V_START_DATE, string DBSOURCE_ID, string WELL_BORE_CODE, string GOV_FCTY_NAME)
        {
            try
            {
                 ...
                 */
                WellBoreInjDay obj = new WellBoreInjDay();
                 ...
                 */
                var result = await _db.WellBoreInjDay
                    .Where(e =>
            ...
            {
                _logger.LogError($"Something went wrong in WellBoreInjDayCompactController: {e.Message}. Stacktrace: {e.StackTrace}");
